...
export default function DaySlot({
  label, sublabel, records, isToday, isSchedule, isActive, onEdit, onCreate, onDelete, onDuplicate, partitionKey,
}) {
      ...
      <div className="flex-1 flex flex-col gap-1">
        {hasRecords ? (
          records.map((record, i) => (
            <RecordRow
              key={partitionKey ? record[partitionKey] ?? i : i}
              record={record}
...
function RecordRow({ record, isSchedule, onEdit, onDelete, onDuplicate }) {
  ...
  const displayStart = p.startTime ? convertTime(p.startTime, storedTz, userTz) : "—";
  const displayEnd   = p.endTime   ? convertTime(p.endTime,   storedTz, userTz) : "—";
  const displayTz    = p.startTime ? tzAbbr(userTz) : (p.timezone ?? "");
    ...
    <div className="relative rounded bg-white/80 border border-[#e4d4e4] px-1.5 py-1 flex flex-col gap-0.5 overflow-hidden min-h-13.5">
      ...
      {isSchedule ? (
        <>
          ...
          <div className="text-[9px] text-[#8b6b8b] leading-tight truncate">{displayTz}</div>
          {(p.vmMenu || p.offerVm) && (
