...
import SettingsPanel from "../Main/Components/SettingsPanel.jsx";
import { useUserSettings } from "../../context/UserSettingsContext.jsx";
import { tzAbbr } from "../Main/utils/tzUtils.js";
...
export default function Header() {
  ...
  const { userTz } = useUserSettings();

  return (
    <header id="app-header" className="bg-rose-900 shadow-md shrink-0">
      <div id="app-header-inner" className="w-full px-6 h-10 flex items-center justify-between">
        <span id="app-header-brand-title" className="text-xl font-semibold text-white tracking-wide">
          Contact Center HQ
        </span>

        <div className="flex items-center gap-3">
          {/* Timezone display + gear */}
          <div className="flex items-center gap-1">
            <span className="text-xs text-rose-200 font-mono select-none">
              {tzAbbr(userTz)}
            </span>
            <SettingsPanel dark />
          </div>
                ...
                <div className="absolute right-0 mt-1 w-48 bg-white rounded shadow-lg z-20 py-2 text-sm text-gray-700">
                  <div className="px-4 py-2 border-b border-gray-100">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      <span className="inline-block bg-rose-100 text-rose-700 px-2 py-0.5 rounded-full font-medium">
                        {DEFAULT_USER.access}
                      </span>
                    </p>
                  </div>
