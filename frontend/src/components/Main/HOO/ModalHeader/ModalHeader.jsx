export default function ModalHeader({ hooConfig, view, formMode, selectedQueue, onBack, onClose }) {
  const modeBadge = {
  ...
  const showBack = view === "form" || view === "week";
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-xl font-bold text-[#3b1a3b]">{hooConfig.title}</h2>
          ...
          <p className="text-sm text-[#5b2d5b] mt-0.5">{hooConfig.description}</p>
        </div>
      </div>
      <button
        type="button"
