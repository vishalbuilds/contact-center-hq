...
import StringField from "../../Table/RecordForm/FieldInput/StringField.jsx";
import DropdownField from "../../Table/RecordForm/FieldInput/DropdownField.jsx";
import BooleanField from "../../Table/RecordForm/FieldInput/BooleanField.jsx";
import IntegerField from "../../Table/RecordForm/FieldInput/IntegerField.jsx";
import DateField from "../../Table/RecordForm/FieldInput/DateField.jsx";
import TimeField from "../../Table/RecordForm/FieldInput/TimeField.jsx";
...
export default function HOOForm({ hooConfig, formMode, initialValues, onDone, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    hooConfig.fields?.forEach((f) => {
      ...
      // In create mode skip empty strings (avoid storing empty attributes).
      // In edit mode include them so the user can clear a field.
    ...
    try {
      if (formMode === "create" || formMode === "duplicate") {
        const res = await fetch(`${BASE(hooConfig.tableName)}/record`, {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-pk": pkId, "x-sk": skId ?? "" },
        ...
        const res = await fetch(
          `${BASE(hooConfig.tableName)}/record/${encodeURIComponent(pkVal)}/${encodeURIComponent(skVal ?? "")}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json", "x-pk": pkId, "x-sk": skId ?? "" },
            body: JSON.stringify(body),
