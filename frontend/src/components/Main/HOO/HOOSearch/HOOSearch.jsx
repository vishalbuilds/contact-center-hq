...
export default function HOOSearch({ hooConfig, onViewAll, onCreateNew, onBulkEdit }) {
  const [searchInput, setSearchInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [phase, setPhase] = useState("idle");
  const [results, setResults] = useState([]);
  const [selected, setSelected] = useState(new Set()); // Set of queueArn strings
  ...
  const toggleSelect = (queueArn) => {
    setSelected((prev) => {
      const next = new Set(prev);
      if (next.has(queueArn)) next.delete(queueArn);
      else next.add(queueArn);
      return next;
    });
  };
  ...
  const handleBulkDelete = async () => {
    ...
    // Remove successfully deleted queues from results regardless of partial failures
        ...
        <div className="max-w-2xl flex flex-col gap-3">
          ...
          {results.map((item) => (
            ...
            >
              <input
                type="checkbox"
                checked={selected.has(item.queueArn)}
