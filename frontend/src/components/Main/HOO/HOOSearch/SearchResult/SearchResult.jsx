...
export default function SearchResult({ queueName, queueArn, onOpen, onDuplicate, onDelete }) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-[#d4c4d4] hover:border-rose-300 transition-colors">
      <div className="min-w-0 flex-1">
        <p className="text-xs text-[#8b6b8b] uppercase tracking-wide leading-tight">Queue Name</p>
        <p className="font-semibold text-sm text-[#3b1a3b]">{queueName}</p>
        <p className="font-mono text-xs text-[#8b6b8b] mt-0.5 truncate">{queueArn}</p>
      </div>
      ...
      <div className="relative ml-3 shrink-0" ref={menuRef}>
        ...
        {menuOpen && !confirmingDelete && (
          <div className="absolute right-0 top-full mt-1 bg-white border border-[#d4c4d4] rounded-xl shadow-lg z-20 py-1 min-w-36 overflow-hidden">
            <button
              type="button"
        ...
        {menuOpen && confirmingDelete && (
          <div className="absolute right-0 top-full mt-1 bg-white border border-red-200 rounded-xl shadow-lg z-20 p-3 min-w-52">
            <p className="text-sm font-semibold text-[#3b1a3b] mb-1">Delete all records?</p>
            <p className="text-xs text-[#8b6b8b] mb-3 truncate">{queueName}</p>
