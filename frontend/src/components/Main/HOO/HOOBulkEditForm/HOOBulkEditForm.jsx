...
function buildDefaults(hooConfig) {
  const vals = {};
  hooConfig.fields?.forEach((f) => { if (!f.hidden) vals[f.id] = f.defaultValue ?? ""; });
  return vals;
}

/*
  HOOBulkEditForm — apply a set of field changes across multiple selected queues.

  The user picks which days (or a date for exceptions), checks which fields
  to overwrite, sets the values, then submits. Each queue×day combination
  is upserted (create or update) and results are shown row-by-row.

  Props:
    hooConfig       — full HOO config from schema.json
    selectedQueues  — [{ queueArn, queueName }]
    onDone          — called when the user clicks Done/Close after completion
    onCancel        — called when the user cancels before submitting
*/
export default function HOOBulkEditForm({ hooConfig, selectedQueues, onDone, onCancel }) {
  ...
  const toggleField = (id) => setApplyFields((prev) => {
    const next = new Set(prev);
    if (next.has(id)) next.delete(id); else next.add(id);
    return next;
  });

  const handleChange = (fieldId, value) => setFormValues((prev) => ({ ...prev, [fieldId]: value }));
  ...
  const handleSubmit = async () => {
    ...
    const toSortKey = (iso) => { const [y, m, d] = iso.split("-"); return `${m}/${d}/${y}`; };
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {progress ? (
          <div className="max-w-lg">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs text-[#8b6b8b] shrink-0">{progress.done} / {progress.total}</span>
            </div>
                ...
                {editableFields.map((field) => {
                  ...
                  return (
                    <div key={field.id} className={`flex gap-3 items-start p-3 rounded-xl border transition-colors ${
                      ...
                      <input type="checkbox" checked={active} onChange={() => toggleField(field.id)}
                      ...
                      <div className={`flex-1 ${!active ? "opacity-40 pointer-events-none select-none" : ""}`}>
                        <Component field={field} value={formValues[field.id]} onChange={handleChange} error={false} />
                      </div>
                    </div>
                  );
                })}
      ...
      <div className="shrink-0 flex items-center justify-between px-8 py-4 border-t border-[#c8b8c8]">
        <button type="button" onClick={isDone ? onDone : onCancel} disabled={progress && !isDone}
