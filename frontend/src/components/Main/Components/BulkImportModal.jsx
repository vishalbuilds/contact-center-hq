...
const MODE_META = {
  get: {
    label: "Get Data",
    color: "blue",
    templateLabel: "GET Template  (queue / record names only)",
    uploadHint: "Upload a CSV with one queue or record name per row",
    submitLabel: (n) => `Fetch ${n} Record${n !== 1 ? "s" : ""}`,
  },
  create: {
    label: "Bulk Create",
    color: "emerald",
...
/*
  BulkImportModal — large full-height modal for bulk Get / Create / Update.

  Props:
    hooConfig    — HOO config object (pass this OR tableConfig, not both)
    tableConfig  — table config object
    initialMode  — "get" | "create" | "update" (skips the mode selector step)
    isAdmin      — controls whether Create and Update modes are selectable
    onClose      — called when the modal should close
*/
export default function BulkImportModal({ hooConfig, tableConfig, initialMode, isAdmin, onClose }) {
  ...
  const handleTemplateDownload = () => {
    let csv, name;
    if (mode === "get") {
      csv = isHoo ? buildHooGetTemplate(config) : buildTableGetTemplate(config);
      name = `${config.id ?? config.tableName}-get-template.csv`;
    } else {
      csv = isHoo ? buildHooTemplate(config) : buildTableTemplate(config);
      name = `${config.id ?? config.tableName}-${mode}-template.csv`;
    }
  ...
  const parseFile = (text, name) => {
    ...
    let parsed;
    if (mode === "get") {
      parsed = parseGetCSV(text);
    } else {
      const res = isHoo ? parseHooCSV(text, hooConfig) : parseTableCSV(text, tableConfig);
      parsed = res.rows;
      if (res.droppedCount > 0) {
        setWarning(`${res.droppedCount} row${res.droppedCount !== 1 ? "s" : ""} skipped — missing primary key.`);
      }
    }
  ...
  const downloadResults = () => {
    const csv = isHoo ? hooRecordsToCSV(result.records, hooConfig) : tableRecordsToCSV(result.records, tableConfig);
    downloadCSV(csv, `${config.id ?? config.tableName}-results.csv`);
  };

  const downloadFailures = () => {
    const csv = failedRowsToCSV(result.results, rows, isHoo, config);
    downloadCSV(csv, `${config.id ?? config.tableName}-failures.csv`);
  };
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-4 top-10 bottom-10 z-70 bg-[#e8e0e8] rounded-2xl shadow-2xl border-l-4 border-l-rose-900 flex flex-col max-w-2xl mx-auto">
        ...
        <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-[#c8b8c8]">
          <div className="flex items-center gap-3">
            ...
            {fileName && <span className="text-xs text-[#8b6b8b] truncate max-w-40">{fileName}</span>}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === "mode" && (
            <div className="flex flex-col gap-3">
              ...
              {Object.entries(MODE_META).map(([id, m]) => {
                const locked = (id === "create" || id === "update") && !isAdmin;
                return (
                  <button
                    key={id}
                    type="button"
                    disabled={locked}
                    onClick={() => { if (!locked) { setMode(id); setStep("file"); } }}
                    className={`w-full text-left px-5 py-4 bg-white rounded-xl border transition-all ${
                      locked
                        ? "border-[#e8d8e8] opacity-50 cursor-not-allowed"
                        : "border-[#d4c4d4] hover:border-rose-400 hover:shadow-sm cursor-pointer group"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <p className={`font-semibold text-sm text-[#3b1a3b] ${!locked && "group-hover:text-rose-800"}`}>{m.label}</p>
                      {locked && <span className="text-[10px] font-semibold text-[#a090a0] bg-[#ede4ed] rounded-full px-2 py-0.5">Admin only</span>}
                    </div>
          ...
          {/* ── Preview — same table layout for all 3 modes ── */}
          {step === "preview" && (() => {
            // Normalise rows into flat objects for display
            // Get mode: rows are strings → wrap into single-column objects
            // Create/Update: rows are objects → flatten payload.* sub-fields
            const lookupKey = isHoo ? config.GSIKey : config.partitionKey;
            const displayRows = mode === "get"
              ? rows.map((name) => ({ [lookupKey]: name }))
              : rows.map((row) => {
                  const flat = {};
                  Object.entries(row).forEach(([k, v]) => {
                    if (k === "payload" && v && typeof v === "object") {
                      Object.entries(v).forEach(([pk, pv]) => { flat[`payload.${pk}`] = pv; });
                    } else {
                      flat[k] = v;
                    }
                  });
            ...
            return (
              <div>
                <p className="text-sm font-medium text-[#3b1a3b] mb-3">
                  {rows.length} row{rows.length !== 1 ? "s" : ""} parsed — ready to submit
                </p>

                <div className="overflow-x-auto rounded-xl border border-[#d4c4d4] bg-white mb-4 max-h-64">
                    ...
                    <tbody>
                      {displayRows.slice(0, 10).map((row, i) => (
                        <tr key={i} className="border-t border-[#e8d8e8]">
                          {headers.map((h) => (
                            <td key={h} className="px-3 py-1.5 font-mono whitespace-nowrap max-w-40 truncate">
                              {String(row[h] ?? "")}
                            </td>
                          ))}
                        </tr>
                      ...
                      {displayRows.length > 10 && (
                        <tr className="border-t border-[#e8d8e8]">
                          <td colSpan={99} className="px-3 py-2 text-[#8b6b8b] text-center">
                            … and {displayRows.length - 10} more rows
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                {(mode === "get" ? result.report : result.results).map((r, i) => (
                  <div key={i} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs ${STATUS_STYLE[r.status] ?? "bg-[#f5f0f5] text-[#8b6b8b]"}`}>
                    ...
                    <span className="flex-1 font-medium truncate">
                      {r.queueName ?? r.pkValue ?? `Row ${(r.rowIndex ?? i) + 1}`}
                      {r.sortKey ? ` (${r.sortKey})` : ""}
                    </span>
